...
const UsersList = ({ onLogout }) => {
  ...
  return (
    ...
    <div className="flex justify-around items-center p-5">
      ...
      />
      <button onClick={onLogout} className="bg-white w-30 p-3 hover:cursor-pointer rounded-full shadow-purple-600">Logout</button>
      </div>
    <div className="users-list-container flex flex-col items-center">
     
      <div className="users-grid">
        {currentUsers.map(user => (
          <div key={user.id} className="user-card min-w-4/4 h-auto p-5 bg-white rounded-lg shadow-lg m-5 ">
            <img src={user.avatar} alt="Avatar" />
            <h3 className="font-semibold">{user.first_name} {user.last_name}</h3>
            <p><span className="font-semibold">Email: </span>{user.email}</p>
            <button className="edit-btn w-30  shadow-2xl px-3 m-6 bg-purple-300 rounded-lg hover:scale-105 duration-300 p-2" ><Link to={`/edit/${user.id}`} >Edit</Link></button>
            <button onClick={() => handleDelete(user.id)} className="edit-btn w-30  shadow-2xl px-3 m-6 bg-purple-300 rounded-lg hover:scale-105 duration-300 p-2">Delete</button>
          </div>
        ))}
      </div>
